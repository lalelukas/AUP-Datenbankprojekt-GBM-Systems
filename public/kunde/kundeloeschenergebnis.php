    ...
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        ...
        <!-- <style>
          ...
          $sql = "SELECT * FROM Kunden WHERE Nachname LIKE '$NAME' OR Kundennummer LIKE '$ID'";
          $result = $conn->query($sql);
          if ($result->num_rows > 0) { 
          ...
          while($row = $result->fetch_assoc()) {
            if ($row["Vorname"] == NULL){
          echo "
          Kunde konnte mit $ID nicht gefunden werden
          ";
            }
            else {
              echo 
              "<h4>Folgender Kunde wird gelöscht: " . $row["Kundennummer"] . "</h4>
              <p> <span class=\"fett\">Vorname:     </span>" . $row["Vorname"] . "</p>
              <p> <span class=\"fett\">Nachname:   </span> " . $row["Nachname"] . "</p>
              <p> <span class=\"fett\">Straße:  </span> " . $row["Strasse"] . "</p>
              <p> <span class=\"fett\">HausNr.:  </span> " . $row["Hausnummer"] . "</p>
              <p> <span class=\"fett\">Postleitzahl:  </span> " . $row["Postleitzahl"] . "</p>
              <p> <span class=\"fett\">Ort:  </span> " . $row["Wohnort"] . "</p>
              <p> <span class=\"fett\">Mail:  </span> " . $row["Mail"] . "</p>
              <br />";
              echo 
              '<form action="kundeloeschenbest.php" method="post">
              <label class=labelID>Kunden-ID: </label> <input type="text" name="ID" value=' . $row["Kundennummer"] . ' readonly /> 
              <p class=sameLine> kann gelöscht werden. </p> <br />
              <input id="submit-btn" type="Submit" value="Löschen bestätigen" />
              </form>';
            }
          }
