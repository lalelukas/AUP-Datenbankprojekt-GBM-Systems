  ...
  <div id="content-wrapper">
    ...
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Ausleihbestätigung</li>
      </ol>


        <!-- Page Content -->
        <h1>Buch ausgeliehen!</h1>
        <hr>
        ...
        <!-- <style>
          ...
          $sql = "UPDATE `buecher` SET `Kunde` = $Kunde WHERE `buecher`.`ID` = $ID;";
          $result = $conn->query($sql);
          $sql = "SELECT * FROM Buecher LEFT JOIN kunden ON buecher.Kunde = kunden.Kundennummer WHERE ID LIKE '$ID'";
          $result = $conn->query($sql);
          if ($result->num_rows > 0) { 
...
echo <<<EOT
          <h4 style=color:green;>Folgendes Buch ist nun ausgeliehen: {$row["Titel"]}</h4><br>
          <h5>ID: {$row["ID"]} </h5><br>
          <h5>ISBN: {$row["ISBN"]} </h5><br>
          <h5>Autor:  {$row["Autor"]} </h5><br>
          <h5>Regal:  {$row["Regal"]} </h5><br>
          <h5>Name:  {$row["Vorname"] } {$row["Nachname"]} </h5><br>
          <h5>KundenID:  {$row["Kundennummer"]} </h5><br>
EOT;
              }
            }
          }
